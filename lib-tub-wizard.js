...
function generateProposals(){
  ...
  // F2(2026-09-06)：人體目標長度（夾回前）；被空間上限壓短時卡上要說
  ...
  PROPS = defs.map(d=>{
    ...
    // F2(2026-09-06)：note 加空間上限壓短提示；stretch 卡被壓短時 why/whyHeight 改文案，避免「Lie flat at」失真
    ...
    return { name:d.name, params, img, dims:`${d.L} × ${d.W} × ${params.H} mm`, depth:d.D, cap:spec.fullVol.toFixed(0), price,
      ...
      note: (function(){
        ...
        if(d.L < want && d.L === T.maxL) return 'Limited by your space';      // F2：空間上限壓短
